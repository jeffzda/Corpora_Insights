#!/usr/bin/env python3
"""
Full corpus failure mode reclassification: retag all 16,931 records with
the revised 7 failure modes (+ no major failure stated). Also asks for
suggested missing categories and an ideal 2-word label per record.

Usage:
    python scripts/batch_fm_full_reclass.py --batch submit
    python scripts/batch_fm_full_reclass.py --batch status
    python scripts/batch_fm_full_reclass.py --batch collect
    python scripts/batch_fm_full_reclass.py --analyze
"""
...
For each record:
- Select the ONE failure mode that best describes what is observed.
- If you think there are failure mode categories missing from the list that would help classify this record more effectively, suggest them.
- Also provide the ideal two-word label that you think most precisely captures the failure in this specific record (even if it matches one of the 8 above).

Respond ONLY with a JSON object (no markdown, no explanation):
{"failure_mode": "technical underperformance", "confidence": 0.85, "missing_categories": [], "ideal_label": "efficiency shortfall"}

If no failure: {"failure_mode": "no major failure stated", "confidence": 0.95, "missing_categories": [], "ideal_label": "no failure"}"""
...
def parse_response(text, record_id):
    ...
    try:
        ...
        return {"failure_mode": "none", "confidence": 0.0,
                "missing_categories": [], "ideal_label": "",
...
def run_analyze():
    ...
    print(f"Records with no missing categories: {n_no_missing} ({n_no_missing/total*100:.1f}%)")

    print(f"\nFailure mode distribution:")
    for fm, count in fm_counts.most_common():
        print(f"  {fm:40s} {count:6d}  ({count/total*100:.1f}%)")

    print(f"\nTop 40 missing categories suggested:")

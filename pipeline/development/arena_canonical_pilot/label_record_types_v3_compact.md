...
Return one JSON object with a single top-level key `a` (assignments) mapping to an array. One object per input record, in input order. Use the **short field names** below — they map to the axis names in the rest of this prompt:

- `id` — the record id
- `o` → `is_occurrence`: `1` for yes, `0` for no
- `m` → `is_mechanism`: `1` for yes, `0` for no
- `s` → `is_specification`: `1` for yes, `0` for no
- `l` → `is_lesson`: `1` for yes, `0` for no
- `r` → `is_recommendation`: `1` for yes, `0` for no
- `v` → `valence`: one of `"pos"`, `"neg"`, `"neu"`, `"nv"` (positive / negative / neutral / no_valence)

Example:

```
{"a":[
{"id":"ARENA-DLV-1234-0001","o":1,"m":0,"s":1,"l":0,"r":0,"v":"neg"},
{"id":"ARENA-DLV-1234-0002","o":0,"m":0,"s":0,"l":1,"r":0,"v":"nv"}
]}
```

No commentary, no markdown fences inside the JSON. Emit one object per input record, in the same order.

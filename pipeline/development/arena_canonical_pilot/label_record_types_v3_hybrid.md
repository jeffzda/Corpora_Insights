...
Return one JSON object with a single top-level key `assignments` mapping to an array. One object per input record, in input order. **Use the full axis names as keys** but compact the values: `1` for yes, `0` for no on the boolean axes; for `valence` use one of `"pos"`, `"neg"`, `"neu"`, `"nv"` (positive / negative / neutral / no_valence).

Emit each record as a single line — no whitespace inside objects beyond what's shown.

```
{"assignments":[
{"id":"ARENA-DLV-1234-0001","is_occurrence":1,"is_mechanism":0,"is_specification":1,"is_lesson":0,"is_recommendation":0,"valence":"neg"},
{"id":"ARENA-DLV-1234-0002","is_occurrence":0,"is_mechanism":0,"is_specification":0,"is_lesson":1,"is_recommendation":0,"valence":"nv"}
]}
```

No commentary, no markdown fences inside the JSON. One object per input record, in the same order.

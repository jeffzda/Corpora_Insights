...
Return ONE line per input record, in the same order as the input. Each line is the record's `id`, a single space, and a 7-character bitstring. No JSON, no markdown fences, no commentary, no preamble, no blank lines.

The 7-character bitstring is positional:

- position 1: `is_occurrence` — `1` for yes, `0` for no
- position 2: `is_mechanism` — `1` for yes, `0` for no
- position 3: `is_specification` — `1` for yes, `0` for no
- position 4: `is_lesson` — `1` for yes, `0` for no
- position 5: `is_recommendation` — `1` for yes, `0` for no
- positions 6-7: `valence` — two-bit code:
  - `00` = `no_valence`
  - `01` = `neutral`
  - `10` = `positive`
  - `11` = `negative`

Example output for two records:

```
ARENA-DLV-1234-0001 1010011
ARENA-DLV-1234-0002 0001000
```

Emit exactly one line per input record. Do not skip any. Do not duplicate any.

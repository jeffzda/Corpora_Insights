# Cluster-to-parent confidence rating

You're shown a candidate cluster-to-parent assignment from a failure-mode taxonomy. For each (cluster, parent) pair, indicate confidence that the cluster's mechanism matches the parent's mechanism criterion.

## Rubric (matches the original assignment task)

Confidence is based on **how cleanly the cluster's mechanism matches the parent's mechanism criterion**:

- **high** — cluster mechanism is an unambiguous instance of the parent's criterion
- **medium** — cluster mechanism plausibly fits but adjacency to other parents or partial match means it is not unambiguous
- **low** — cluster mechanism only weakly satisfies the criterion, or a different parent would describe it more naturally
...
Strict JSONL, one record per cluster, in input order. No commentary.

```json
{"cluster_id": "cNNN", "parent_id": "pNN", "confidence": "low|medium|high", "rationale": "<≤25 words>"}
```

## Cluster→parent pairs to rate

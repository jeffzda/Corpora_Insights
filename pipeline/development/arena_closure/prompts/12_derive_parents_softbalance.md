...
7. **Soft population balance.** All else equal, prefer parent definitions that subsume **similar total volumes of records** across their child clusters. Extreme imbalance in record-count populations across parents may signal that one parent is too coarse (absorbing what should be multiple parents) or another too fine (when adjacent narrow mechanisms could merge into a coherent broader parent). Use this as a sanity-check on parent definition, not a hard target — some mechanism families are inherently broad in the corpus (generic informational gaps, planning inadequacy) and others inherently narrow (chicken-and-egg deadlocks, equity outcomes). Accept population variation that reflects genuine differences in mechanism prevalence; the constraint exists to push back against extreme tails, not to force equal populations. If natural mechanism boundaries produce uneven populations, keep the boundaries.

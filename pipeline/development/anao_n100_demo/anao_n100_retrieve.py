#!/usr/bin/env python3
"""Poll the ANAO N=100 batch and parse results when ready.

Usage: python3 corpora/anao/n100_demo/output/anao_n100_retrieve.py
"""
from __future__ import annotations
import json, sys, time
from pathlib import Path
from collections import Counter
import anthropic

ROOT = Path('/home/jeffzda/broadlearnings')
OUT_DIR = ROOT / 'corpora/anao/n100_demo/output'
BATCH_ID_FILE = OUT_DIR / 'anao_n100_batch_id.txt'
MANIFEST_FILE = OUT_DIR / 'anao_n100_manifest.json'
RESULTS_RAW = OUT_DIR / 'anao_n100_results_raw.jsonl'
RECORDS_OUT = OUT_DIR / 'anao_n100_records.jsonl'
META_OUT = OUT_DIR / 'anao_n100_extraction_meta.json'


def parse_records(text: str) -> list[dict] | None:
    t = text.strip()
    if t.startswith('```'):
        t = t.split('\n', 1)[1]
        if t.endswith('```'): t = t.rsplit('```', 1)[0]
    s, e = t.find('{'), t.rfind('}')
    if s < 0 or e <= s: return None
    try:
        d = json.loads(t[s:e+1])
        return d.get('records', [])
    except Exception:
        return None


def main():
    if not BATCH_ID_FILE.exists():
        sys.exit(f'no batch_id at {BATCH_ID_FILE}; run anao_n100_extract.py first')
    batch_id = BATCH_ID_FILE.read_text().strip()
    manifest = json.load(MANIFEST_FILE.open())
    docs_by_id = {d['slug'][:60]: d for d in manifest['docs']}
    print(f'batch: {batch_id}')

    client = anthropic.Anthropic()
    batch = client.messages.batches.retrieve(batch_id)
    print(f'status: {batch.processing_status}  counts: {batch.request_counts.model_dump()}')

    if batch.processing_status != 'ended':
        print('batch not ready yet — re-run later')
        return

    # Stream results
    records_total = []
    raw_lines = []
    in_tok = out_tok = 0
    for result in client.messages.batches.results(batch_id):
        custom_id = result.custom_id
        if result.result.type != 'succeeded':
            print(f'  ! {custom_id}: {result.result.type}')
            continue
        msg = result.result.message
        in_tok += msg.usage.input_tokens
        out_tok += msg.usage.output_tokens
        text = ''.join(b.text for b in msg.content if hasattr(b, 'text'))
        raw_lines.append(json.dumps({'custom_id': custom_id, 'text': text}))
        recs = parse_records(text)
        if recs is None:
            print(f'  ! {custom_id}: parse failed')
            continue
        meta = docs_by_id.get(custom_id, {})
        for r in recs:
            r['_doc_slug'] = meta.get('slug')
            r['_year_tabled'] = meta.get('year_tabled')
            r['_portfolio'] = meta.get('portfolio')
            r['_entity'] = meta.get('entity')
            r['_sector'] = meta.get('sector')
            r['_era'] = meta.get('era')
        records_total.extend(recs)

    RESULTS_RAW.write_text('\n'.join(raw_lines))
    with RECORDS_OUT.open('w') as f:
        for r in records_total:
            f.write(json.dumps(r) + '\n')

    # Cost
    cost = in_tok / 1e6 * 5 * 0.5 + out_tok / 1e6 * 25 * 0.5
    print(f'\nrecords extracted: {len(records_total)}')
    print(f'tokens: {in_tok:,} in / {out_tok:,} out')
    print(f'batch cost: ${cost:.2f}')

    # Distribution stats
    by_era = Counter(r.get('_era') for r in records_total)
    by_sig = Counter(r.get('significance') for r in records_total)
    print(f'\nby era: {dict(by_era)}')
    print(f'by significance: {dict(by_sig)}')

    META_OUT.write_text(json.dumps({
        'batch_id': batch_id,
        'n_docs_input': len(manifest['docs']),
        'n_records': len(records_total),
        'records_per_doc_avg': round(len(records_total) / max(len(manifest['docs']),1), 1),
        'input_tokens': in_tok, 'output_tokens': out_tok,
        'cost_usd': round(cost, 4),
        'era_distribution': dict(by_era),
        'significance_distribution': dict(by_sig),
    }, indent=2))
    print(f'\nwrote {RECORDS_OUT}')
    print(f'wrote {META_OUT}')


if __name__ == '__main__':
    main()

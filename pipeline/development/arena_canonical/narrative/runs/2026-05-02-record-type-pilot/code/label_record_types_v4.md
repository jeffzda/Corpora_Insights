...
**`yes`** if the record describes something that was **set by the project** or is **inherent to equipment employed by the project** — including:

- Project parameters: scope, schedule, budget, location list, organisational structure, program design and stated aims
- Equipment specifications: nameplate capacity, operating limits (e.g. RPF, voltage), model/manufacturer, design architecture, control strategy
- Measured magnitudes that characterise the project itself: total capacity deployed, number of sites, programme reach (e.g. % of stations hybridised, MW added, MWh produced per tranche)

**A record can be `is_specification: yes` AND `is_occurrence: yes` simultaneously.** The axes are independent. If a record describes an equipment swap such as "low-load diesel generators tolerating 90% RPF replaced traditional 60%-RPF generators", both are `yes`: it is an occurrence (the swap happened) AND a specification (the RPF values are equipment limits inherent to the equipment). Do not tag `is_specification: no` just because outcome or change-over-time language is present — read past the framing for embedded parameter content.

**`no`** if the record describes any of:

- **Pure outcomes with no embedded parameter content** ("the trial was successful")
- **Mechanisms or causal pathways** without specification content
- **Prescriptions or recommendations**
- **Environmental conditions and operating-context constraints** — characteristics of the *operating environment* the project encountered but did not set: limited mobile coverage, travel difficulties, animal hazards, local supplier limitations, infrastructure deficits, weather risks, remoteness. These describe the environment the project worked *in*, not what the project *is* or what its equipment *does*.

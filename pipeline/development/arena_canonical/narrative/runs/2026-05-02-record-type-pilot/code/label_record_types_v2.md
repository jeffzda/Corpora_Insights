...
- ❌ Tagging "X was identified as required" as `no_valence` because the record itself is structurally a recommendation. Read the underlying situation: if X being required implies that not having X was a problem, the underlying situation is `negative`.
- ❌ Tagging a lesson "in similar situations, do Y" as `no_valence` because the lesson is phrased prescriptively. Read the underlying situation: if the lesson exists because not doing Y caused issues, the underlying situation is `negative`.
- ✅ Tagging a record as `no_valence` only when neither outcome nor underlying situation is positively or negatively coloured — pure neutral facts about a system's design or program structure with no reference to problems or successes.

...
- Same finding stated with slightly different emphasis from two records of the same source document.
- One record is a slightly more detailed restatement of the other.
- **Aspect-distinct records of one occurrence**: a record describing the *cause* of a delay, a record describing the *mechanism*, a record describing the *intervention*, a record describing the *outcome*, and a record describing the *lesson learnt* should all share one event_id when they refer to the same singular occurrence. Different mechanism *families* across these records is expected and desired — downstream clustering uses that span as evidence.
- **Near-paraphrases of the same finding** — even when one carries an extra clarifying clause.

## When to SPLIT (different events)

These cases look superficially similar but represent distinct atomic findings:

1. **Different INSTANCES of similar issues** — Phase 1 commissioning delay vs Phase 2 commissioning delay; two separate grid faults at different times; two distinct contracting incidents at different milestones.
2. **Different MAGNITUDES, dates, equipment IDs, or milestones attached to ostensibly similar findings** — different numbers usually mean different events.
3. **Different RECOMMENDATIONS or design decisions on the same subsystem** — two distinct prescriptive findings about how to do something differently are typically distinct events.
4. **Generalised principles vs project-specific occurrences** — "in off-grid systems generally, ride-through capability is critical" is a different event from "the wind turbine intermittently shut down due to internal control functions in this project." Generalised principles emit their own events.
